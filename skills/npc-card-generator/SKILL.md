...
- 不从零手写简化版卡牌 HTML。必须复用本 Skill 目录中的 `npc-card-prototype.html`、`scripts/` 和 `assets/`。
- 如果 `npc-card-prototype.html`、`assets/images/card/` 或 `assets/fonts/` 缺失，必须阻塞并报告资源缺失，不得降级生成低保真替代品。
...
**选项 C：使用默认空白图**
> 使用默认空白占位，不得使用 prototype 自带的旧人物图。用户可以在网页中上传自己的图片。

**图片处理**：
- 复制到输出目录的 `assets/` 文件夹
- 统一重命名为 `portrait-[npc-name].png`
- 调整尺寸（最大 1024x1024）
- 当前 NPC 的 Step 2 / Step 8 形象图优先级最高。
- 找不到当前 NPC 形象图时，HTML 必须显示空白占位和上传图片控件，不得默认显示示例人物图。
...
**强制实现路径**：
1. 以 Skill 目录中的 `npc-card-prototype.html` 为模板生成卡牌页面。
2. 必须复制 `assets/images/card/`、`assets/fonts/` 和 `scripts/` 到输出目录，保持相对路径可用。
3. 优先运行 `scripts/generate_card_from_bible.py` 生成输出；如果脚本不可用，必须手动复用 prototype 的 Canvas 渲染代码和素材路径。
4. 禁止临时手写 CSS-only 或简化 canvas 版本替代 prototype。
5. 不得复制或默认显示 prototype 的示例人物图（如 `portrait-feifei.jpg`）。只有找到当前 NPC 的形象图，或用户显式提供图片时，才可设置 portrait。
6. 生成前检查以下资源：
   - `npc-card-prototype.html`
   - `assets/images/card/card_bg.png`
   - `assets/images/card/synchro_border.png`
...
**依赖资源**：
- 35+ 张卡牌素材 PNG（从 NIKKE 卡牌生成器提取）
- 4 种字体文件（Pretendard-Bold, AireExterior, AbolitionTest-Regular, Azonix）
- Canvas 2D API（浏览器原生支持）

**资源位置**：
- Prototype: `npc-card-prototype.html`
- Card assets: `assets/images/card/`
- Fonts: `assets/fonts/`
- Rendering helpers: `scripts/card.js`, `scripts/util.js`

这些资源是高保真卡牌输出的必要条件。缺失时不要生成劣化 HTML。
